...
function SentimentHeatmap() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const uniqueConversations = Array.from(new Set(enriched.map(d => d.conversation))).sort();
  ...
  useEffect(() => {
    ...
    if (selectedConversation !== "All") {
      filtered = filtered.map(o => ({
          ...
          count: o.messages.filter(m => m.conversation === selectedConversation).length,
          messages: o.messages.filter(m => m.conversation === selectedConversation)
        }));
    }
  ...
  useEffect(() => {
    ...
    setSortedData(filteredData.slice().sort((a, b) => b.count - a.count));
  }, [filteredData]);
  ...
  useEffect(() => {
    drawBar();
  }, [sortedData, minSentiment, maxSentiment]);
        ...
        <div className="popup-inner">
          ...
          <svg width={svgWidth} height={svgHeight}>
            <g transform={`translate(${margin.left},${margin.top})`}>
              <g transform={`translate(0,${(height+ margin.top + margin.bottom)/2 })`} ref={node => d3.select(node).call(d3.axisBottom(x).ticks(5))} />
              <g ref={node => d3.select(node).call(d3.axisLeft(y).ticks(5))} />
              <path d={line(parsedData)} fill="none" stroke="steelblue" strokeWidth="2" />
              {parsedData.map((d, i) => (
                <circle key = {i} a-key={i} cx={x(d.date)} cy={y(d.sentiment)} r={3} fill="black" onMouseOver={(event, _) => 
                  {
                    let tooltip1 = d3.select("#tooltip1")
                    let cd = parsedData[event.target.getAttribute('a-key')]
                    d3.select("#tooltip1").transition().duration(200).style("opacity", 1);
                    tooltip1.html(`<strong>${new Date(cd.timestamp).toString()}</strong><br/>Sender: ${cd.sender}<br/>Message: ${cd.message}`)
                      .style("left", `${event.pageX + 10}px`)
                      .style("top", `${event.pageY - 28}px`);
                  }
                } />
          ...
          <div id='tooltip1' className='tooltip' style={{ opacity: 0 }}></div>
